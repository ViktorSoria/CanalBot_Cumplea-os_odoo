...
class PaymentAcquirer(models.Model):
    ...
    def _create_missing_journal_for_conekta_acquirers(self, company=None):
        ...
        acquirer_names = ['conekta','conekta_oxxo','conekta_spei']
    ...
    def conekta_spei_get_form_action_url(self):
        return '/payment/conekta_spei/feedback'
    
    def conekta_spei_form_generate_values(self, tx_values):
        self.ensure_one()
        conekta_tx_values = dict(tx_values)
        conekta_tx_values.update({'phone': tx_values.get('partner_phone'),})
        return conekta_tx_values
...
class PaymentTransaction(models.Model):
    ...
    @api.model
    def _conekta_spei_form_get_tx_from_data(self, data):
        reference, amount, currency_name = data.get('reference'), data.get('amount'), data.get('currency_name')
        tx = self.search([('reference', '=', reference)])
    ...
    def _check_conekta_form_get_invalid_parameters(self, data):
        invalid_parameters = []
    ...
    def _conekta_oxxo_form_get_invalid_parameters(self, data):
        return self._check_conekta_form_get_invalid_parameters(data)
        

    def _conekta_spei_form_get_invalid_parameters(self, data):
        return self._check_conekta_form_get_invalid_parameters(data)
    ...
    def _conekta_spei_form_validate(self, data):
        _logger.info('Validated conekta_spei payment for tx %s: set as pending' % (self.reference))
        conekta.api_key = self.acquirer_id.conekta_secret_key_test if self.acquirer_id.state=='test' else self.acquirer_id.conekta_secret_key 
        params = self.create_params('conekta_spei')
        try:
            response = conekta.Order.create(params)
        except conekta.ConektaError as error:
            err_val = ''
    ...
    def _conekta_s2s_validate_tree(self, tree):
        ...
        if status == 'paid':
            ...
            is_spei = self.acquirer_id.provider=='conekta_spei'
            ...
            if not is_spei:
                vals.update({'conekta_oxxo_barcode': base64.encodestring(
